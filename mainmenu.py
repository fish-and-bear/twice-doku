...
def intro():
    ...
    logo = pg.image.load('pictures/intrologo.png')
    rect = logo.get_rect()
    rect.center = (Var.WIDTH/2, Var.HEIGHT/2)
    screen.blit(logo, rect)
    pg.display.flip()
    audioHandler.play(pg.mixer.Sound('songs/sfx/intro.ogg'))
...
animation_database['menu'] = load_animation('animations/menu', [7, 7, 7])
animation_database['highscores'] = load_animation(
    'animations/highscores', [10, 10])
animation_database['display_highscores'] = load_animation('animations/displayhighscores', [
...
def options():
    ...
    resolution = pgui.elements.UIDropDownMenu(options_list=['1000x600', '1280x768'],
        ...
        manager=manager)
    volume_slider = pgui.elements.UIHorizontalSlider(relative_rect=pg.Rect(Var.WIDTH/2.5, Var.HBTN1+Var.bottommargin, Var.WIDTH/5, Var.HEIGHT/16),
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_ESCAPE:
                    running = False
        try:
            scores = fileHandler.json_open(Var.scoresfile)
            try:
...
# Go to game
def play(level, highscore):
    running = True
    while running:
        twicedoku.startgame(level, highscore)
        menu()
...
def load_music():
    ...
    for filename in os.listdir(path):
        if filename.endswith(".ogg"):
            songs.append(pg.mixer.Sound(os.path.join(path, filename)))
        else:
            continue
    return songs
    ...
    try:
        if os.stat(Var.gridfile).st_size > 0:
            if os.stat(Var.timefile).st_size > 0:
                stats = fileHandler.json_open(Var.timefile)
                level = stats["level"]
            else:
                level = levels[0]
        else:
...
def menu():
    ...
    displayscore = displayHighscore(level)
    is_running = True
    i = 0
    while is_running:
        # Play a list of songs in the background
        if len(audioHandler.backgroundQueue) == 0:
            audioHandler.queue(random.choice(menu_music),
