...
DEFAULT_GCOLOR = None
DEFAULT_OCOLOR = "black"
DEFAULT_SCOLOR = "black"
...
class _GetsurfOptions(_Options):
	...
	def resolvetags(self):
		if self.fontname is None: self.fontname = DEFAULT_FONT_NAME
		if self.fontsize is None: self.fontsize = DEFAULT_FONT_SIZE
		if self.sysfontname is None: self.sysfontname = DEFAULT_SYSFONT_NAME
		if self.bold is None: self.bold = DEFAULT_BOLD
		if self.italic is None: self.italic = DEFAULT_ITALIC
		if self.underline is None: self.underline = DEFAULT_UNDERLINE
		if self.color is None: self.color = DEFAULT_COLOR
		if self.background is None: self.background = DEFAULT_BACKGROUND
		if self.gcolor is None: self.gcolor = DEFAULT_GCOLOR
		if self.ocolor is None: self.ocolor = DEFAULT_OCOLOR
		if self.scolor is None: self.scolor = DEFAULT_SCOLOR
...
class _GetfontOptions(_Options):
	...
	def getfontpath(self):
		if self.fontname is not None:
			return os.path.join("assets", "fonts", self.fontname)
		else:
			return self.sysfontname
...
def getfont(**kwargs):
	...
	if options.sysfontname is not None:
		...
		try:
			font = pygame.font.Font(fontpath, options.fontsize)
		except IOError:
			print(f"Unable to read font file: {fontpath}")
			# Fallback to default system font
			font = pygame.font.SysFont(None, options.fontsize)
...
def _splitbytags(text, tagspec0, color0, underlinetag, boldtag, italictag, colortag):
    if isinstance(colortag, tuple):
        colortag = dict(colortag)
    colortag = { k: _resolvecolor(v, color0) for k, v in colortag.items() }
    
    # Convert tags to strings if they're tuples
    def ensure_string(tag):
        if isinstance(tag, tuple):
            return tag[0] if tag else None
        return tag

    underlinetag = ensure_string(underlinetag)
    boldtag = ensure_string(boldtag)
    italictag = ensure_string(italictag)
